...
- **Every GitHub Actions `uses:` must pin to a full commit SHA with the semantic version in a trailing comment** — `uses: actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1  # v7.0.1`. Never a tag or branch ref: a tag is mutable, so re-pointing it silently changes what runs in CI.
  - The SHA is the security boundary; the comment is what makes the pin reviewable and what Dependabot rewrites when it bumps the action. A pin without one is a pin nobody can review.
  - Applies to new workflows and to any `uses:` line you touch. `pre-commit.yml`, `publish.yml`, `pytest.yml` and `version_check.yml` are correctly SHA-pinned but still missing the comment — backfill tracked in issue #104.
